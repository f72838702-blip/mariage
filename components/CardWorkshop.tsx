...
export default function CardWorkshop() {
  ...
  return (
    <section className="relative flex min-h-screen flex-col items-center px-4 pb-20 pt-28">
      <style>{`
        @media print {
      ...
      <div className="no-print w-full max-w-xl text-center">
        ...
        <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row">
          <input
            ...
            maxLength={80}
            className="h-11 w-full flex-1 rounded-full border border-sand bg-white/80 px-5 text-sm text-ink outline-none placeholder:text-ink-soft/60 focus:border-gold"
          />
          <div className="flex gap-2">
            <button
              type="button"
              onClick={copyLink}
              className="btn-outline h-11 whitespace-nowrap"
              title="Copier le lien personnalisé de cette carte"
            >
              {copied ? <Check size={16} className="text-sage-dark" /> : <Copy size={16} />}
              {copied ? "Copié !" : "Copier le lien"}
            </button>
            <button type="button" onClick={() => window.print()} className="btn-primary h-11 whitespace-nowrap">
              <Printer size={16} />
              Imprimer
            </button>
          </div>
        ...
        <p className="mt-3 text-xs leading-relaxed text-ink-soft">
          ...
          <span className="break-all font-medium text-ink">{shareUrl}</span>
        </p>
      </div>
      ...
      <div className="card-stage mt-12 flex w-full justify-center px-2 py-10">
        <InvitationCard
          ...
          className="invitation-card w-[min(92vw,540px)] rounded-md shadow-[0_40px_90px_-30px_rgba(44,62,53,0.35),0_10px_30px_-12px_rgba(44,62,53,0.18)]"
        />
      </div>

      <p className="no-print mt-6 max-w-md text-center text-xs leading-relaxed text-ink-soft">
        Astuce : ajoutez <code className="rounded bg-cream px-1.5 py-0.5">?a=Nom+de+l&apos;invité</code> à
